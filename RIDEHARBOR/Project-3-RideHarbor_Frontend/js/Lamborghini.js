...
// Lamborghini mobile scroll glass effect
...
// =============================
// =============================================
// LamborghiniBooking — handles Rent buttons,
// login check, modal open, and booking submit
// =============================================
class LamborghiniBooking {
    constructor() {
        ...
        // Desktop Sign In button in header
        this.signinLink = document.querySelector(".signin a");
        this.init();
    }
    init() {
        // Update header sign in button if already logged in
        ...
        // Wire Sign In / Profile click on header
        if (this.signinLink) {
        ...
        // Mobile profile icon click (the .signin div itself)
        if (this.signinDiv) {
            this.signinDiv.addEventListener("click", (e) => {
                // Avoid double-firing on desktop where the <a> tag handles it
        ...
        if (this.closeLoginBtn) {
            this.closeLoginBtn.onclick = () => this.loginModal.style.display = "none";
        }
        if (this.closeBookingBtn) {
            this.closeBookingBtn.onclick = () => this.bookingModal.style.display = "none";
        }
        // Close profile modal
    ...
    handleRentClick() {
        if (!this.loggedIn) {
            // Show login first, store pending car for after login
            this.loginModal.style.display = "flex";
        } else {
    ...
    async verifyOtp() {
        ...
        try {
            ...
            if (data.status === "success") {
                ...
                // Restore gold glow on page load if already logged in
                if (this.loggedIn && this.signinDiv) {
                    this.signinDiv.classList.add("logged-in");
                }
                // Mobile gold glow — same as Home.js
    ...
    async submitBooking() {
        ...
        const payload = {
            ...
            pickupTime,
            returnTime,
            amount: 150000 // default; you can make dynamic later
        };
        try {
